package com.reven.plugs.ipaddr;
...
public class IPSeeker {
	...
	private IPSeeker() {
		...
		b4 = new byte[4];
		b3 = new byte[3];
		String path = System.getProperty("user.dir") + "/deploy_config/ip_address.dat";
		try {
			File file = new File(path);
			if (!file.exists()) {
				// 兼容本地开发
				path = System.getProperty("user.dir") + "/src/main/resources/deploy_config/ip_address.dat";
			}
	...
	public List<IPEntry> getIPEntriesDebug(String s) {
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (null != loc) {
					if ((null != loc.country && loc.country.indexOf(s) != -1)
							|| (null != loc.area && loc.area.indexOf(s) != -1)) {
